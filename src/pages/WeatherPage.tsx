...
export function WeatherPage() {
  ...
  const { weather, source, error } = useWeatherData(settings);

  return (
    <GlassPanel className="h-full rounded-[2rem] p-8 text-white">
      <div className="grid h-full grid-cols-[0.9fr_1.1fr] gap-8">
        <div className="flex flex-col justify-center">
          <p className="text-2xl font-semibold text-electric-400">{weather.city}</p>
          <div className="mt-4 flex items-center gap-6">
            <CloudSun size={118} className="text-sky-300" />
            <div>
              <p className="text-8xl font-bold">{cToDisplay(weather.temperature, settings.temperatureUnit)}</p>
              <p className="mt-2 text-3xl text-white/72">{weather.condition}</p>
              <p className="mt-3 text-sm text-white/42">
                {source === "apple" ? "Apple WeatherKit data" : "Mock weather data"}
                {error ? ` · ${error}` : ""}
              </p>
            </div>
          </div>
          <div className="mt-10 grid grid-cols-3 gap-4">
            <div className="rounded-2xl bg-white/8 p-5">
              ...
              <p className="text-2xl font-semibold">{cToDisplay(weather.feelsLike, settings.temperatureUnit)}</p>
            </div>
            <div className="rounded-2xl bg-white/8 p-5">
            ...
            <div className="rounded-2xl bg-white/8 p-5">
              ...
              <p className="text-2xl font-semibold">{weather.wind}</p>
            </div>
          </div>
        </div>
